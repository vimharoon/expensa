    ...
    res => {
      if (res[0].email > 0) {
        ...
        return clbk({
          ...
          status: 4092,
          message:
            "Le nom d'utilisateur existe déjà. veuillez ressaisir vos informations",
        })
      }
      ...
      mysql.query(q, (error, results, fields) => {
        ...
        // };
        results.message =
          'vous êtes bien enregistré, veuillez confirmer votre adresse email pour vous connecter'
        clbk(results)
      })
    },
    data.email,
  ...
  authHelpers.getUserInfo(res => {
    if (!res.length) {
      return clbk({
        error: true,
        message:
          "Cette adresse email n'existe pas. veuillez verifier que votre email est correcte",
      })
    } else {
      const q = `UPDATE
                    users
      ...
      mysql.query(q, (error, results, fields) => {
        ...
        clbk({
          ...
          message: `Un mail vient de vous être envoyé à l\'adresse: ${
            res[0].email
          }`,
        })
      })
